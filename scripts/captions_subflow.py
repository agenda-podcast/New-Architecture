...
def find_caption_file(audio_path: Path, video_path: Path) -> Optional[Path]:
    """Locate a captions sidecar generated by the pipeline.

    Supported conventions (prefer JSON if present, because it can carry speaker tags):
      - <audio>.captions.json
      - <audio>.captions.srt
      - <video>.captions.json
      - <video>.captions.srt

    Also normalizes the legacy ".script" suffix.
    """
    candidates = []
    ...
    if audio_path.stem.endswith('.script'):
        stem = audio_path.stem[:-len('.script')]
        candidates.append(audio_path.with_name(stem + '.captions.json'))
        candidates.append(audio_path.with_name(stem + '.captions.srt'))
    if video_path.stem.endswith('.script'):
        stem = video_path.stem[:-len('.script')]
        candidates.append(video_path.with_name(stem + '.captions.json'))
        candidates.append(video_path.with_name(stem + '.captions.srt'))
...
def maybe_burn_captions(
    ...
    This wrapper now delegates to scripts/captions/burner.py, which supports:
      - gender-based TikTok-style glow per host
    ...
    if not captions and not has_titles:
        print("  ⓘ No captions file and no image title timeline; skipping post-process overlays")
        return True

    # Validate captions only if present (SRT only).
    if captions and captions.suffix.lower() == '.srt':
        ok_srt, _sample_t = _validate_srt_against_video(captions, video_path)
        if not ok_srt:
            print("  ⚠ Captions file has no visible cues within video timeline; continuing with titles-only")
    ...
    cfg = CaptionBurnConfig(
        ...
        bottom_margin_fraction=float(os.environ.get('CAPTIONS_BOTTOM_MARGIN_FRACTION', '0.14')),
