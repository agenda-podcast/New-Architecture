...
def _detect_frame_png() -> Optional[Path]:
    """Match burner frame discovery so gating doesn't skip frame-only overlays."""
...
def find_caption_file(audio_path: Path, video_path: Path) -> Optional[Path]:
    ...
    Supported conventions (either next to the video or next to the audio):
      - <stem>.captions.srt
      - <stem>.captions.json

    We also normalize a trailing ".script" suffix in stems produced by some TTS flows.
    """
    ...
    # Normalize ".script" suffix (audio stems can end with ".script")
    if audio_path.stem.endswith('.script'):
        base = audio_path.with_name(audio_path.stem[:-7])  # strip '.script'
        candidates.extend([
...
def maybe_burn_captions(
    ...
    frame_png = _detect_frame_png()
    has_frame = bool(frame_png)

    if not captions and not has_titles and not has_frame:
        print("  ⓘ No captions/titles/frame overlays detected; skipping post-process overlays")
        return True

    # Validate SRT captions only (JSON captions are validated downstream by burner).
    if captions and captions.suffix.lower() == '.srt':
        ok_srt, _sample_t = _validate_srt_against_video(captions, video_path)
        if not ok_srt:
            print("  ⚠ Captions SRT has no visible cues within video timeline; continuing with titles/frame-only")
