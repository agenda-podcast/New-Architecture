...
# New post-processing burner (supports image titles + gender glow) 
from captions.burner import burn_captions_subflow as burn_overlays_subflow, CaptionBurnConfig


def find_captions_srt(audio_path: Path, video_path: Path) -> Optional[Path]:
    """Locate an SRT captions sidecar, if present."""
    candidates: List[Path] = []
    candidates.append(video_path.with_suffix('.captions.srt'))
    candidates.append(audio_path.with_suffix('.captions.srt'))
    # Common alternate names
    candidates.append(Path(str(video_path) + '.captions.srt'))
    candidates.append(Path(str(audio_path) + '.captions.srt'))
    candidates.append(video_path.with_suffix('.srt'))
    candidates.append(audio_path.with_suffix('.srt'))

    # Normalize optional ".script" suffix.
...
def find_captions_json(audio_path: Path, video_path: Path) -> Optional[Path]:
    """Locate a JSON captions sidecar, if present.

    Preferred format produced by the pipeline: <audio>.captions.json
    """
    candidates: List[Path] = []
    candidates.append(audio_path.with_suffix('.captions.json'))
    candidates.append(video_path.with_suffix('.captions.json'))
    candidates.append(Path(str(audio_path) + '.captions.json'))
    candidates.append(Path(str(video_path) + '.captions.json'))

    # Normalize optional ".script" suffix.
    if audio_path.stem.endswith('.script'):
        candidates.append(audio_path.with_name(audio_path.stem[:-len('.script')] + '.captions.json'))
    if video_path.stem.endswith('.script'):
        candidates.append(video_path.with_name(video_path.stem[:-len('.script')] + '.captions.json'))
...
# Back-compat: keep the old name, but prefer SRT (used only for optional validation).
def find_caption_file(audio_path: Path, video_path: Path) -> Optional[Path]:
    """Locate captions file (SRT preferred)."""
    return find_captions_srt(audio_path, video_path) or find_captions_json(audio_path, video_path)
...
def maybe_burn_captions(
    ...
    """Post-process overlays in-place.

    Delegates to scripts/captions/burner.py, which supports:
      - gender-based TikTok-style glow per host (A/B)
      - image title overlays from <video>.image_titles.json
      - optional static PNG frame overlay from repo assets/ or Assets/
      - preserving audio (no re-TTS or remux needed)

    Important: This function intentionally supports both SRT and JSON caption sidecars.
    """
    # Detect sidecars (for logging/validation only). The burner will also auto-discover.
    captions_srt = find_captions_srt(audio_path, video_path)
    captions_json = find_captions_json(audio_path, video_path)
    has_titles = video_path.with_suffix('.image_titles.json').exists()

    if not captions_srt and not captions_json and not has_titles:
        print("  ⓘ No captions sidecar and no image title timeline; skipping post-process overlays")
        return True

    # Validate SRT only if present (JSON captions are already structured).
    if captions_srt:
        ok_srt, _sample_t = _validate_srt_against_video(captions_srt, video_path)
        if not ok_srt:
            print("  ⚠ Captions SRT has no visible cues within video timeline; continuing with titles-only / JSON captions")

    w, h = _probe_video_resolution(video_path)
    if w <= 0 or h <= 0:
        # Fallback to common portrait resolution
        w, h = 1080, 1920

    fps = int(os.environ.get('CAPTIONS_FPS', '30'))
    style = (style_name or os.environ.get('CAPTIONS_STYLE', 'tiktok')).strip().lower()

    cfg = CaptionBurnConfig(
        enabled=True,
        style_preset=style,
        renderer=os.environ.get('CAPTIONS_RENDERER', 'auto').strip().lower(),
        font_size_fraction=float(os.environ.get('CAPTIONS_FONT_SIZE_FRACTION', '0.055')),
        bottom_margin_fraction=float(os.environ.get('CAPTIONS_BOTTOM_MARGIN_FRACTION', '0.20')),
        left_right_margin_fraction=float(os.environ.get('CAPTIONS_LEFT_RIGHT_MARGIN_FRACTION', '0.06')),
        tiktok_glow_color=os.environ.get('CAPTIONS_TIKTOK_GLOW_COLOR', '#C0C0C0'),
        hide_speaker_names=os.environ.get('CAPTIONS_HIDE_SPEAKER_NAMES', 'true').strip().lower() in ('1','true','yes','on'),
    )

    ok = burn_overlays_subflow(video_path=video_path, audio_path=audio_path, width=w, height=h, fps=fps, config=cfg)
    if not ok:
        require = os.environ.get('REQUIRE_BURNED_CAPTIONS', 'true').strip().lower() in ('1', 'true', 'yes', 'on')
        if require:
            print("  ✗ Overlay burn-in failed and REQUIRE_BURNED_CAPTIONS is enabled")
            return False
        print("  ⚠ Overlay burn-in failed; continuing without burned overlays")
        return True
