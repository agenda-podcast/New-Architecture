...
def collect_images_for_topic(
    ...
    # Collect image URLs from search results with pagination
    image_urls = []
        ...
        while len(query_results) < query_target and start_index <= 91:  # Max start is 91 (to get results 91-100)
            ...
            try:
                ...
                result = service.cse().list(
                    ...
                    imgSize='XLARGE'  # Prefer large images (uppercase required by API)
                ).execute()
                ...
                if page_items:
                    for item in page_items:
                        if 'link' in item:
                            query_results.append(item['link'])
                            logger.info(f"  Found image {len(query_results)}: {item.get('title', 'No title')[:50]}")
    ...
    logger.info(f"Total unique image URLs collected: {len(unique_image_urls)}")
    ...
    downloaded_images = []
    for i, url in enumerate(unique_image_urls[:num_images]):
